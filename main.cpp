...
int main() {
    initwindow(500, 600); // создание окна для граф приложения

    setfillstyle(SOLID_FILL, WHITE); // создание стиля заливки
    bar(0, 0, 500, 600); // белый фон

    setcolor(LIGHTGREEN); // цвет для текста
    char* text = new char[6];//("Tree");
    outtextxy(200, 10, text); // вывод текста на картинку


    setcolor(BROWN); // коричневый ствол
    rectangle(190, 450, 250, 490); // прямоуг
    moveto(190, 450);
    lineto(220, 150);
    lineto(250, 450);


    setfillstyle(LTSLASH_FILL, BROWN); // зальем
